...
//TODO: Hash and Encode with BASE32
    ...
    public RealmIdentity getRealmIdentity(final Principal principal) {
        try{
            if(principal instanceof NamePrincipal){ return getRealmIdentity(principal.getName(), false); }
            else{ return RealmIdentity.NON_EXISTENT; }
    ...
    public ModifiableRealmIdentity getRealmIdentityForUpdate(final Principal principal){
        try{
            if(principal instanceof NamePrincipal){ return getRealmIdentity(principal.getName(), true); }
            else{ return ModifiableRealmIdentity.NON_EXISTENT; }
        }catch(GeneralSecurityException e){
            e.printStackTrace();
            return null;
        }
    ...
    static class Identity implements ModifiableRealmIdentity {
        ...
        private static final String ENCRYPTION_FORMAT = "secretkey_base64";
...
//      TODO: Decrypt Attributes when fetching
        ...
        private void writeIdentity(final XMLStreamWriter streamWriter, final LoadedIdentity newIdentity) throws XMLStreamException, GeneralSecurityException {
            streamWriter.writeStartDocument();
            streamWriter.writeCharacters("\n");
            streamWriter.writeStartElement("identity");
            // Continue to write using 1.0 as not using any features added in 1.0.1
